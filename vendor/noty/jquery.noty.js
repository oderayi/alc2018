...
(function ($) {
  const NotyObject = {
    ...
    _build() {
      // Generating noty bar
      const $bar = $('<div class="noty_bar"></div>').attr('id', this.options.id);
      ...
      if (this.options.buttons) {
        ...
        $.each(this.options.buttons, (i, button) => {
    ...
    show() {
      ...
      if ($.inArray('click', self.options.closeWith) > -1) {
        self.$bar.css('cursor', 'pointer').one('click', (evt) => {
      ...
      if ($.inArray('button', self.options.closeWith) > -1) {
        self.$closeButton.one('click', (evt) => {
          self.stopPropagation(evt);
          self.close();
        });
      }
  ...
  $.noty.consumeAlert = function (options) {
    window.alert = function (text) {
      if (options) { options.text = text; } else { options = { text }; }
  ...
  $(window).resize(() => {
    $.each($.noty.layouts, (index, layout) => {
...
window.noty = function noty(options) {
  ...
  jQuery.each(options, (key, value) => {
    if (old_to_new[key]) {
      using_old++;
      const _new = old_to_new[key].split('.');

      if (!options[_new[0]]) options[_new[0]] = {};

      options[_new[0]][_new[1]] = (value) || function () {
      };
      delete options[key];
    }
  });
  ...
  if (options.buttons) {
    jQuery.each(options.buttons, (i, button) => {
      if (button.click) {
        using_old++;
